...
def get_places(api_key, latitude, longitude, radius, types):
    headers = {
        "Content-Type": "application/json",
    ...
    response = requests.post("https://places.googleapis.com/v1/places:searchNearby", headers=headers, json=data)
    if response.status_code == 200:
...
# Streamlit App
st.title("Wettervorhersage und Aktivitätsvorschläge")
st.write("Geben Sie eine Stadt ein, um die Wettervorhersage und passende Aktivitäten zu erhalten.")

city_name = st.text_input("Stadtname:")
start_date = st.date_input("Startdatum:")
end_date = st.date_input("Enddatum:")
activity_radius = st.slider("Aktivitäten-Radius (in Metern):", 1000, 20000, 5000)
activities = st.multiselect("Aktivitätstypen:", ["park", "museum", "restaurant", "tourist_attraction"])

api_key = st.text_input("Google Places API Key:", type="password")

if st.button("Vorhersage anzeigen und Aktivitäten vorschlagen"):
        ...
        if isinstance(geocoding_results, list) and geocoding_results:
            ...
            if isinstance(weather_data, dict):
                ...
                for date, data in weather_data.items():
                    ...
                    st.write("---")
                st.write("Aktivitätenvorschläge:")
                places = get_places(api_key, latitude, longitude, activity_radius, activities)
                if isinstance(places, list):
                    for place in places:
                        st.write(f"Name: {place['displayName']}")
                        st.write(f"Adresse: {place['formattedAddress']}")
                        st.write(f"Typen: {', '.join(place['types'])}")
                        st.write("---")
                else:
                    st.error("Fehler beim Abrufen der Aktivitäten.")
            else:
                st.error("Fehler beim Abrufen der Wetterdaten.")
        else:
            st.error("Fehler beim Abrufen der Geodaten.")
    else:
        st.error("Bitte geben Sie einen Stadtnamen und den API-Schlüssel ein.")
